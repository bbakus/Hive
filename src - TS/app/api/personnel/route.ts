...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data: BackendPersonnel[] = await response.json();
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!response.ok) {
      const errorBody = await response.text();
      return NextResponse.json(
        { error: `Failed to create personnel in Python backend: ${response.status} ${response.statusText}`, details: errorBody },
        { status: response.status }
      );
    }
