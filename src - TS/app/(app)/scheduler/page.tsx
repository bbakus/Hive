...
export default function SchedulerPage() {
  ...
  useEffect(() => {
    ...
    const filteredPersonnel = personnelList.filter(p => 
        personnelIdsInProjectEvents.has(p.personnelId) && 
        PHOTOGRAPHY_ROLES.includes(p.role as typeof PHOTOGRAPHY_ROLES[number]) && p.role !== "Client"
    );
    setProjectPersonnel(filteredPersonnel);
    
    setSelectedPersonnelIds(prev => prev.filter(id => filteredPersonnel.some(p => p.personnelId === id)));
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const selectedPersonnelNames = selectedPersonnelIds.map(id => {
      const person = projectPersonnel.find(p => p.personnelId === id);
