...
export default function PostProductionPage() {
  const { events, isLoadingEvents, shotRequestsByEventId } = useEventContext();
  const { selectedProject, isLoadingProjects } = useProjectContext();
  const { personnelList } = usePersonnelContext();
  ...
  const getPersonnelName = (personnelId?: string): string => {
    ...
    return personnelList.find(p => p.personnelId === personnelId)?.name || "Unknown Personnel";
  };

  // Filter events for selected project
  const filteredEvents = selectedProject ? events.filter(event => event.projectId === selectedProject.id) : events;
  ...
  useEffect(() => {
    ...
    const derivedTasks: KanbanTask[] = filteredEvents
      ...
      .map((event, index) => {
        ...
        const photoPersonnelIds = event.assignedPersonnelIds?.filter(pid => {
            const p = personnelList.find(person => person.personnelId === pid);
        ...
        const shotsForEvent = shotRequestsByEventId[event.id] || [];
        ...
        return {
          ...
          eventName: `${selectedProject?.name || 'Unknown Project'} - ${event.name}`,
          photographerName: photographerDisplay,
          ingestionJobId: `event-ingest-${event.id}`,
          reportUrl: `/events/${event.id}/shots`,
  ...
  }, [filteredEvents, isLoadingEvents, isLoadingProjects, isLoadingSettings, selectedProject, shotRequestsByEventId]);

  // Get editors from personnel list
  const availableEditors = personnelList.filter(p => 
    p.role.toLowerCase().includes('editor') || 
    p.role.toLowerCase().includes('photographer') ||
    p.role.toLowerCase().includes('producer')
  );

  const getEditorName = (editorId?: string | null) => {
    if (!editorId) return null;
    const editor = availableEditors.find(p => p.personnelId === editorId);
    return editor?.name || 'Unknown Editor';
  };
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (filterOnlyMyTasks) {
      currentTasks = currentTasks.filter(task => task.assignedEditorId === currentUserId);
    }
    if (filterHideCompleted) {
      currentTasks = currentTasks.filter(task => task.status !== "completed");
    }
    return currentTasks;
  }, [tasks, filterTaskStatus, filterAssignedEditorId, filterOnlyMyTasks, filterHideCompleted, currentUserId]);
    ...
    setTasks(prevTasks =>
      prevTasks.map(task => {
        if (task.id === taskId) {
          ...
          const editorName = getEditorName(finalAssignedEditorId) || user?.name || 'System Action';
  ...
  const handleSimulateNewIngestion = () => {
    ...
    const randomImageCount = Math.floor(Math.random() * 150) + 50;
                ...
                <Select value={filterAssignedEditorId} onValueChange={setFilterAssignedEditorId}>
                    ...
                    <SelectContent>
                        ...
                        {availableEditors.map(editor => (
                            <SelectItem key={editor.personnelId} value={editor.personnelId}>{editor.name}</SelectItem>
                        ))}
              ...
              <Card className="border-0 bg-transparent h-full">
                ...
                <CardContent className="p-3 space-y-3 min-h-[200px]">
                  ...
                  {tasksByColumn[column.id]?.map((task) => {
                    ...
                    const isCurrentUserAssigned = task.assignedEditorId === currentUserId;
                        ...
                        <CardContent className="p-3 pt-1 text-xs">
                          ...
                          <div className="mt-2.5 flex flex-wrap gap-1.5">
                            {task.status === 'ingestion' && !task.assignedEditorId && (
                              <Button size="xs" variant="ghost" onClick={() => handleTaskAction(task.id, 'culling', currentUserId, 'claim_culling_task')} className="text-xs text-accent">
                                Start Culling
                              </Button>
                            )}
                            ...
                            {task.status === 'culling' && isCurrentUserAssigned && (
                             ...
                             {task.status === 'color' && !task.assignedEditorId && (
                               <Button size="xs" variant="ghost" onClick={() => handleTaskAction(task.id, 'color', currentUserId, 'claim_color_task')} className="text-xs text-accent">
                            ...
                            {task.status === 'review' && (
                                <>
                                  <Button size="xs" variant="ghost" onClick={() => handleTaskAction(task.id, 'completed', currentUserId, 'approve_review')} className="text-xs text-green-600 dark:text-green-400">
                                      Approve & Complete
                                  </Button>
