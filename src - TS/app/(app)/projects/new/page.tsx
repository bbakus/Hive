...
// Define proper project roles that users can select from


// Simplified structure for available personnel in the wizard
interface Step2AvailablePersonnel {
  id: string;
...
export default function NewProjectWizardPage() {
  ...
  const [currentAvailablePersonnel, setCurrentAvailablePersonnel] = useState<Step2AvailablePersonnel[]>([]);
  const [isAddPersonnelDialogOpen, setIsAddPersonnelDialogOpen] = useState(false);
  const [notifyTeam, setNotifyTeam] = useState(true);

  // Update personnel list when PersonnelContext data loads
  useEffect(() => {
    if (!isLoadingPersonnel && personnelList.length > 0) {
      const availablePersonnel: Step2AvailablePersonnel[] = personnelList.map(p => ({
        id: p.personnelId,
        personnelId: p.personnelId,
        name: p.name,
        capabilities: [...PROJECT_ROLES],
        projectRole: ""
      }));
      setCurrentAvailablePersonnel(availablePersonnel);
    }
 ...
 useEffect(() => {
  ...
  // Location autocomplete removed for now - can be added back later if needed
  ...
  const onSubmit: SubmitHandler<ProjectWizardFormDataInternal> = async (data) => {
    try {
  ...
  const currentUserId = user?.id || "unknown_user"; 
      ...
      await addProject(projectDataForContext, data.organizationId, currentUserId);
