...
export default function EventsPage() {
  ...
  const handleEventSubmit = async (data: EventFormDialogData, existingEventId?: string) => {
    ...
    if (editingEvent && currentEventId) {
      try {
        ...
        await updateEvent(currentEventId, fullUpdatePayload);
        toast({
          title: "Event Updated",
        ...
        toast({
          title: "Error",
          description: `Failed to update "${data.name}". Please try again.`,
          variant: "destructive",
        });
        return; // Don't proceed with quick shots if event update failed
      }
    } else {
